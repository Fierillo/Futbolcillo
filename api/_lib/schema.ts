...
export function ensureSchema() {
  ...
  schemaReadyPromise = (async () => {
    ...
    await query`create table if not exists matches (id text primary key, challenge_id text not null, mode text not null, status text not null, home_pubkey text not null, away_pubkey text not null, current_state jsonb not null default '{}'::jsonb, rematch_requested_by text, rematch_requested_at timestamptz, rematch_match_id text, created_at timestamptz not null default now(), updated_at timestamptz not null default now())`;
